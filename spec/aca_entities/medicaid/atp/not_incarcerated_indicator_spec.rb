...
RSpec.describe ::AcaEntities::Medicaid::Atp::NotIncarceratedIndicator,  dbclean: :around_each do
  ...
  let(:optional_params) { { metadata: "pe123" } }
  ...
  context 'invalid parameters' do
    context 'with empty parameters' do
      it 'should list error for every required parameter' do
        expect { described_class.new }.to raise_error(Dry::Struct::Error, /:value is missing in Hash input/)
      end
    end

    context 'with optional parameters only' do
      it 'should list error for every required parameter' do
        expect { described_class.new(optional_params) }.to raise_error(Dry::Struct::Error, /:value is missing in Hash input/)
      end
    end
  end

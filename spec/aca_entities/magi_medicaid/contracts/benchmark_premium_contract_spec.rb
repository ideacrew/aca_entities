  ...
  context 'invalid params' do
    context 'no params' do
      it 'should return failure' do
        expect(subject.call({}).errors.to_h).to eq({ lcsp_premiums: ['is missing'],
                                                     health_only_slcsp_premiums: ['is missing'] })
      end
    end

    context 'invalid value for monthly_premium' do
      let(:bad_params) { input_params.merge({ lcsp_premiums: 'Test' }) }

      it 'should return failure' do
        expect(subject.call(bad_params).errors.to_h).to eq({ lcsp_premiums: ['must be an array'] })
      end
    end
  end
end

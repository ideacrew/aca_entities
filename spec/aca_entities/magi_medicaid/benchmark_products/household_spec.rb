...
RSpec.describe ::AcaEntities::MagiMedicaid::BenchmarkProducts::Household, dbclean: :after_each do
  ...
  describe 'with valid arguments' do
    ...
    before do
      validated_params = AcaEntities::MagiMedicaid::Contracts::BenchmarkProducts::HouseholdContract.new.call(household)
    ...
    it 'should match all the input keys of household' do
      expect(@result.to_h.keys).to eq(household.keys)
    end

    it 'should match all the input keys of health_product_reference' do
      expect(@result.to_h[:health_product_reference].keys).to eq(health_product_reference.keys)
    end

    it 'should match all the input keys of dental_product_reference' do
      expect(@result.to_h[:dental_product_reference].keys).to eq(dental_product_reference.keys)
    end

    it 'should match all the input keys of member' do
      expect(@result.to_h[:members].first.keys).to eq(member.keys)
    end

    it 'should match all the input keys of applicant_reference' do
      expect(@result.to_h[:members].first[:applicant_reference].keys).to eq(member[:applicant_reference].keys)
    end
  end
end

...
RSpec.describe AcaEntities::Ledger::Contracts::TaxHouseholdContract do
  ...
  let(:id) { '12345' }
  let(:exchange_assigned_id) { 787_878 }
  ...
  let(:required_params) { { exchange_assigned_id: exchange_assigned_id, aptc_amount: aptc_amount, start_on: start_on } }
  ...
  context 'Calling the contract with no params' do
    let(:error_message) do
      { exchange_assigned_id: ['is missing'], aptc_amount: ['is missing'], start_on: ['is missing'] }
    end
    it 'should pass validation' do
      result = subject.call({})

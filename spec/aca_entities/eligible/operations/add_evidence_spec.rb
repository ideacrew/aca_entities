...
RSpec.describe AcaEntities::Eligible::AddEvidence do
  ...
  let(:required_params) do
    {
      ...
      evidence: evidence_params.first.merge(title: 'childcare subsidy evidence2')
    }
  end
  ...
  context 'Initializing with required params' do
    it 'should return evidence and eligibility' do
      ...
      expect(output.first).to be_a AcaEntities::BenefitSponsors::BenefitSponsorships::ShopOsseEligibility::AdminAttestedEvidence
    ...
    it 'should return eligibility with newly added evidence' do
      ...
      expect(eligibility.evidences.map(&:title)).to include('childcare subsidy evidence2')
    end
  end
end

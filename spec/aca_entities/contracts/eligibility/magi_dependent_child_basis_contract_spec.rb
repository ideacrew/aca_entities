...
require 'spec_helper'
require 'aca_entities/contracts/eligibility/magi_dependent_child_basis_contract'

RSpec.describe ::AcaEntities::Contracts::Eligibility::MagiDependentChildBasisContract, dbclean: :around_each do

  it 'should be a container-ready operation' do
    expect(subject.respond_to?(:call)).to be_truthy
  end

  let(:required_params) {{ covered_code: "200" }}

  let(:all_params) do
    required_params.merge(
      covered_indicator: true,
  ...
  context 'failure case' do
    context 'without all required params' do
      ...
      it 'should return failure with errors' do
        ...
        expect(result.errors.to_h).to eq({ :covered_code => ["is missing"] })
      end
    end

    context 'invalid input for status code' do
      it 'should return failure with error message' do
        result = subject.call({ covered_code: 200 })
        expect(result.failure?).to be_truthy
        expect(result.errors.empty?).to be_falsy
        expect(result.errors.to_h).to eq({ :covered_code => ["must be a string"] })
      end
    end

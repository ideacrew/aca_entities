...
require 'spec_helper'
require 'aca_entities/contracts/eligibility/chip_pregnancy_category_basis_contract'

RSpec.describe ::AcaEntities::Contracts::Eligibility::ChipPregnancyCategoryBasisContract,  dbclean: :around_each do
  ...
  let(:all_params) do
    required_params.merge(
      ...
      category_determination_date: DateTime.now,
  ...
  context 'failure case' do
    ...
    context 'invalid input for determination_date' do
      it 'should return error message' do
        result = subject.call(required_params.merge(category_determination_date: "end date"))
        expect(result.errors.to_h).to eq({ :category_determination_date => ["must be a date time"] })
      end
    end
  end
end

...
RSpec.describe AcaEntities::Atp::Functions::BuildApplication do
  ...
  context "with valid xml containing pregnant applicant" do
    ...
    it "should return pregnancy end date for pregnant applicant with future end date" do
      expect(@result[:applicants][1][:pregnancy_information][:pregnancy_end_on]).to eq(Date.today + 1)
    end
    it "should not return pregnancy end date for past end date" do
      expect(@result[:applicants][2][:pregnancy_information][:pregnancy_end_on]).to be nil
    end
  end
end

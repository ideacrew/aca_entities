  ...
  module Contracts
    ...
    class EmployerSponsoredInsuranceInformationContract < Dry::Validation::Contract
      ...
      params do
        ...
        optional(:employee_status_code).maybe(Types::EMPLOYEE_STATUS_CODE_KINDS)
        optional(:changes_esi_status).maybe(:bool)
        optional(:retiree_plan_coverage).maybe(:bool)
        optional(:lcsop_coverage_code).maybe(Types::LCSOP_COVERAGE_CODE_KINDS)
        optional(:lcsop_end_date).maybe(:date)
        optional(:lcsop_cost_code).maybe(Types::LCSOP_COST_CODE_KINDS)

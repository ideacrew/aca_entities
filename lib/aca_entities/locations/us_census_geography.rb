...
module Locations
  ...
  class UsCensusGeography < ActiveRecord::Base
    ...
    FILE_HEADER = { summary_level: { length: 3 } }
    HEADERS = [:summary_level, :state_code, :county_code, :county_subdivision_code, :place_code, :consolidated_city_code, :area_name]
    ...
    def self.load_file(file_name)
      strip_leading_zeros = lambda { |field, _| return unless field.present?
      ...
      CSV.foreach(file_name, headers: HEADERS).with_index do |row, i|
        ...
        when STATES_SUMMARY_LEVEL
          ...
          new_record = (row.to_h).merge!({ postal_code: state_pair[1],
                                           state: state_pair.first,
                                           state_abbr: state_pair.second })
      ...
      puts "#{count}"
      # binding.pry
      #       self.collection.with.bulk_write([ { insert_many: requests }])

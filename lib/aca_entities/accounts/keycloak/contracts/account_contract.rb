      ...
      module Contracts
        ...
        class AccountContract < Contract
          ...
          params do
            ...
            optional(:attributes).hash(AcaEntities::Accounts::Keycloak::Contracts::AttributesContract.params)
          ...
          rule(:profiles).each do |index:|
            values.to_h[:profiles[index]]&.each_pair do |attr_key, attr_val|
              ...
              key([*path, attr_key]).failure({ text: 'error', code: result.errors.to_h })
            end
          end
        end
      end

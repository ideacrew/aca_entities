        ...
        class IntegratedCaseTransformContract < Curam::ApplicationContract
            ...
            if key? && value
              applications_array = value.inject([]) do |hash_array, appl_hash|
                if appl_hash.is_a?(Hash)
                  result = ::Curam::ParticipantRoles::ProductDeliveryCaseTransformContract.new.call(appl_hash)
                  if result&.failure?
          ...
          rule(:participant_roles) do
            if key? && value
              applications_array = value.inject([]) do |hash_array, appl_hash|
                if appl_hash.is_a?(Hash)
                  result = ::Curam::ParticipantRoles::ParticipantRoleTransformContract.new.call(appl_hash)
                  if result&.failure?

...
module AcaEntities
  module Medicaid
    module Curam
      module People
        # Person contract
        class PersonContract < Curam::ApplicationContract
          params do
          ...
          rule(:identifiers) do
            if key? && value
              key.failure(text: "Only one identifier should be of key: #{Types::PrimaryKeyKind}") if value.select do |each_id|
                                                                                                       each_id[:key] == Types::PrimaryKeyKind
                                                                                                     end.count > 1

...
module Ehs
  module Applications
    class ApplicationContract < Ehs::ApplicationContract

      params do
        optional(:source_system_key).maybe(:string)
        required(:ids).array(:hash)
        required(:status).filled(Ehs::Types::ApplicationStatusKind)
        required(:status_code).filled(Ehs::Types::ApplicationStatusCodeKind)

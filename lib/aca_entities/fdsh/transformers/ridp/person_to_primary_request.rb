      ...
      module Ridp
        ...
        class PersonToPrimaryRequest < ::AcaEntities::Operations::Transforms::Transform
          ...
          add_key 'StreetName', function: ->(v) {
            binding.pry
            person_addresses = v.resolve('addresses').item
            home_address = person_addresses.detect {|address| address['kind'] == 'home'}
            home_address.address_1
          }

          map 'hbx_id', 'hbx_id', memoize: true, visible: false
          add_key 'test_hbx_id', function: ->(v) {
            binding.pry
            v.resolve('hbx_id').item
          }
